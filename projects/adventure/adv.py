...
# utility function to grab opposite/inverse direction to backtrack a path.
...
# need this gentleman to be declared so that I can start my while condition.
visited_rooms = set()

# this holds the traversals that are done through the world map/roomGraph dict.
traversalPath = []

# create a Stack.
moves = Stack()

# begin loop by comparing length of the visited rooms set to the length of the roomGraph dict - 1 for all test cases.
while len(visited_rooms) < len(roomGraph) - 1:
    # get possible exits.
    exits = player.currentRoom.getExits()
    # create a list to hold directions.
    directions = []
    # checking to ensure all possible exits are here. expect n e s w at first.
    print(f"exits: {exits}")
    for exit in exits:
        print(f"exit: {exit}")
        # going through our exists and ensuring that it has not been visited previously to populate our directions list.
        if player.currentRoom.getRoomInDirection(exit) not in visited_rooms:
            print(f"--> exit not in visited_rooms: {exit} triggered")
            # add it to the directions list
            directions.append(exit)
    # add current room to the visited_rooms set.
    print(f"player.currentRoom: {player.currentRoom}")
    visited_rooms.add(player.currentRoom)
    # begin checking the directions that we are able to move in, select a random direction, add to Stack, and move in that direction.
    if len(directions) > 0:
        print(f"directions {directions}")
        print(f"-------------> len(directions): {len(directions)}")
        # create a random_direction with the length of however long our list is to exhaust all options.
        random_direction = random.randint(0, len(directions) - 1)
        print(f"random_direction------------->: {random_direction}")
        moves.push(directions[random_direction])
        print(f"directions[random_direction]: {directions[random_direction]}")
        # get out of dodge. let's go.
        player.travel(directions[random_direction])
        # leave a note that says we were there.
        traversalPath.append(directions[random_direction])
    # in the case we get stuck somewhere, in other words there are no directions to move in, we will move in the opposite direction of what got us stuck.
    else:
        print(f"len(directions) in else: {len(directions)} <================")
        # grab the last move.
        im_stuck = moves.pop()
        print(f"im_stuck: {im_stuck}")
        # travel to the opposite direction.
        print(f"get_reverse(im_stuck) {get_reverse(im_stuck)}")
        player.travel(get_reverse(im_stuck))
        # append that as part of the traversal.
        traversalPath.append(get_reverse(im_stuck))
